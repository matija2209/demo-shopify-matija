...
export const formatSkincareDataToText = (data: SkincareFormData): string => {
  ...
  const basicProfile = [
    ...
    `Age Group: ${
      data.ageGroup === 'TWENTIES'
        ? '20s'
        : data.ageGroup === 'THIRTIES'
        ? '30s'
        : data.ageGroup === 'FORTIES'
        ? '40s'
        : data.ageGroup === 'FIFTIES'
        ? '50s'
        : '60s+'
    }`,
  ...
  const environmental = [
    `Daily Sun Exposure: ${
      data.sunExposure === 'RARE'
        ? 'Rare'
        : data.sunExposure === 'MODERATE'
        ? 'Moderate'
        : 'Frequent'
    }`,
  ...
  const lifestyle = [
    ...
    `Sleep Duration: ${
      data.sleepPatterns === 'LESS_THAN_6_HRS'
        ? 'Less than 6 hours'
        : data.sleepPatterns === '6_TO_8_HRS'
        ? '6-8 hours'
        : 'More than 8 hours'
    }`,
  ].join('\n');
  ...
  const budget = [
    `Monthly Budget: $${
      data.monthlyBudget === 'LOW'
        ? '50'
        : data.monthlyBudget === 'MID_RANGE'
        ? '150'
        : '200'
    }`,
    data.hasPreferencesEthical && data.ethicalPreferences
      ? `Ethical Preferences: ${data.ethicalPreferences
          .map((pref) =>
            pref === 'CRUELTY_FREE'
              ? 'Cruelty Free'
              : pref === 'VEGAN'
              ? 'Vegan'
              : pref === 'SUSTAINABLE_PACKAGING'
